...
<body onLoad="startExcute()">
	<div> 
		<section class="login_bt">
			<div class="divoi">
				<p>我们已发送短信验证码至<?php echo ($phone); ?>，请在输入框内填写验证码，若未收到请耐心等候~</p>
			</div>
			
			<form id="form" action="?act=phonezc" method="post">
				<div class="divoi_odi">
					<div class="login_td" id="login_dt">
						<input type="number" placeholder="请输入验证码" id="captcha" name="qryzm" onKeyDown="return handleEnter(this, event)">
					</div>
					<div class="login_od" id="login_do">
						<a href="?act=sphone&phone=<?php echo ($phone); ?>" id="getCaptcha" style="display:block" disabled="true">60秒重新发送</a>
					</div>
				...
				<div class="asied">
					<div class="aisde_odi">
						<input type="text" placeholder="请输入昵称" id="inviteCode" name="username">
					</div>
				</div>
			...
			<aside class="aised_dis" style="display: block;">
				<ul>
					<li>· 在验证码有效期1分钟</li>
					<li>· 密码请输入6-16个英文字母、数字组合~</li>
					<li>· 注册成功后昵称和注册邮箱均可作登录名~</li>
				</ul>
				<p>*详情请咨询13732136280~</p>
			</aside>
			<div class="odi"></div>
			<aside>
				<input type="submit" value="注册" id="regButton" class="login_btn" onClick="return checkCaptcha()">
			</aside>	
		   </form>
		</section>
	</div>
...
<script>
		...
		function startExcute(){
			getCaptcha.innerHTML= wait + "秒重新发送";
